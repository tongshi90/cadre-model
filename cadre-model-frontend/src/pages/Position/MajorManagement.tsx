...
const MajorManagement = () => {
  ...
  const [treeData, setTreeData] = useState<MajorType[]>([]);
  const [hoveredNodeId, setHoveredNodeId] = useState<number | null>(null);
  const [expandedKeys, setExpandedKeys] = useState<Set<number>>(new Set([0])); // 默认展开根节点
  const [modalVisible, setModalVisible] = useState(false);
  const [modalMode, setModalMode] = useState<'create' | 'edit'>('create');
  const [currentNode, setCurrentNode] = useState<MajorType | null>(null);
  const [parentId, setParentId] = useState<number | null>(null);
  const [deleteConfirm, setDeleteConfirm] = useState<MajorType | null>(null);
  const [form] = Form.useForm();

  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await majorApi.getTree();
      const data = response.data.data || [];
      // 添加根节点 "全部专业"
      const rootData: MajorType = {
        id: 0,
        name: '全部专业',
        parent_id: null,
        sort_order: 0,
      ...
      console.error('Failed to fetch major tree:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  // 新增节点
  const handleCreate = (node: MajorType) => {
    setModalMode('create');
    setCurrentNode(null);
  ...
  // 编辑节点
  const handleEdit = (node: MajorType) => {
    if (node.id === 0) return;
    setModalMode('edit');
    setCurrentNode(node);
  ...
  // 删除节点
  const handleDelete = (node: MajorType) => {
    if (node.id === 0) return;
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await majorApi.delete(deleteConfirm.id);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (modalMode === 'edit' && currentNode) {
        await majorApi.update(currentNode.id, {
          name: values.name,
  ...
  // 计算子节点数量
  const countChildren = (node: MajorType): number => {
  ...
  // 渲染树节点
  const renderTreeNode = (node: MajorType, level: number = 0): React.ReactNode => {
    ...
    <div className="department-page">
      ...
      <Modal
        title={modalMode === 'create'
          ? (parentId === null ? '新增岗位类型' : '新增专业类型')
          : (parentId === null ? '编辑岗位类型' : '编辑专业类型')
        }
        open={modalVisible}
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item
            label={parentId === null ? '岗位类型名称' : '专业类型名称'}
            name="name"
            rules={[{ required: true, message: `请输入${parentId === null ? '岗位类型' : '专业类型'}名称` }]}
          >
            <Input placeholder={`请输入${parentId === null ? '岗位类型' : '专业类型'}名称`} />
          </Form.Item>

          {parentId !== null && modalMode === 'create' && (
            <div className="parent-info">
              <span className="parent-label">所属岗位类型：</span>
          ...
          <Form.Item label="描述" name="description">
            <Input.TextArea
              rows={3}
              placeholder={`请输入${parentId === null ? '岗位类型' : '专业类型'}描述（选填）`}
              showCount
              maxLength={200}
            />
          </Form.Item>
